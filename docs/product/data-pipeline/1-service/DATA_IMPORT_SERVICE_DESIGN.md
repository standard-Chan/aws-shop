# DataImportService 설계 문서

**작성일**: 2026-04-16  
**대상**: AWS Shop 상품 데이터 적재 파이프라인  
**범위**: JSONL 한 줄을 받아 파싱하고, 루트 엔티티와 하위 엔티티를 생성한 뒤 FK를 연결하여 DB 저장까지 수행하는 적재 서비스 설계

---

## 1. 목적

이 문서는 `DataImportService`가 수행해야 할 책임을 정의한다.
...
## 4. DataImportService 책임

`DataImportService`는 insert 유스케이스의 오케스트레이터다.
...
`DataImportService`는 JSON line 1개를 받아서:
