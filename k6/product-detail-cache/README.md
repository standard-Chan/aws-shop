...
```bash
TPS=1 DURATION=5s bash k6/product-detail-cache/run-with-mysql-metrics.sh uniform
```

## 실행 방법

캐싱 도입 전후의 MySQL 부하 지표까지 함께 비교하려면 `run-with-mysql-metrics.sh`를 사용한다. 결과 파일은 `k6/product-detail-cache/results/{테스트유형}-tps{TPS}-duration{DURATION}.json` 형식으로 생성된다.
...
같은 시나리오, 같은 TPS, 같은 duration, 같은 seed로 캐싱 적용 전후를 각각 실행한다.
...
DURATION=5m \
SEED=20260717 \
RESULT_FILE=k6/product-detail-cache/results/before-cache-zipf-tps1000-duration5m.json \
bash k6/product-detail-cache/run-with-mysql-metrics.sh zipf
```
...
DURATION=5m \
SEED=20260717 \
RESULT_FILE=k6/product-detail-cache/results/after-cache-zipf-tps1000-duration5m.json \
bash k6/product-detail-cache/run-with-mysql-metrics.sh zipf
```

비교할 핵심 값:

- MySQL query request count 감소율: `mysql.queryRequests.questions`
- MySQL SELECT 감소율: `mysql.queryRequests.comSelect`
- MySQL QPS 감소율: `mysql.queryRequests.qpsByQuestions`
- MySQL CPU 감소율: `mysql.cpu.normalizedProcessCpuPercent`
- 응답 시간 개선: `k6.latencyMs.p95`, `k6.latencyMs.p99`
- 안정성 변화: `k6.failureRatePercent`, `k6.successRatePercent`
...
| `RESULT_FILE` | 통합 결과 JSON 파일 경로. 지정하면 기본 파일명 규칙보다 우선한다. | `{RESULT_DIR}/{SCENARIO}-tps{TPS}-duration{DURATION}.json` |
...
| 시나리오 | 결과 파일 |
| --- | --- |
| 균등 요청 | `k6/product-detail-cache/results/uniform-tps{TPS}-duration{DURATION}.json` |
| 일반 편향 | `k6/product-detail-cache/results/zipf-tps{TPS}-duration{DURATION}.json` |
| 이벤트 집중 | `k6/product-detail-cache/results/event-tps{TPS}-duration{DURATION}.json` |
...
```json
{
  "title": "zipf-tps1000-duration5m",
  "scenario": "zipf",
  "durationSeconds": 300.1,
  "k6": {
    "totalRequests": 300000,
    "successfulRequests": 300000,
    "failedRequests": 0,
  ...
  "mysql": {
    "queryRequests": {
      ...
      "queries": 2100500,
      "comSelect": 1800000,
      "qpsByQuestions": 6997.6,
      "sampledQpsByQuestions": {
        "avg": 7010.2,
        "min": 6800.1,
        "max": 7200.4
      }
    ...
    "cpu": {
      "available": true,
      "normalizedProcessCpuPercent": 62.4,
      "sampledNormalizedProcessCpuPercent": {
        "avg": 61.9,
        "min": 55.2,
        "max": 70.8
      }
...
- `rawProcessCpuPercent`: `mysqld` 프로세스의 전체 논리 CPU 기준 누적 사용률이다. 멀티코어에서는 100%를 넘을 수 있다.
- `normalizedProcessCpuPercent`: `rawProcessCpuPercent`를 논리 processor 수로 나눈 값이다. 전후 비교에는 이 값을 우선 사용한다.
